...
class MeasurementController extends GetxController {
  ...
  void _onData(NoiseReading noiseReading) {
    if (noiseReading.meanDecibel.isFinite) {
      ...
      currentDb.value = 0.0; // Valor padrão para casos inválidos
    }
    update();
  }
  ...
  Future<void> saveMeasurement({
    ...
    required String userEmail, // Adicionado o email do usuário
  }) async {
    ...
    if (isWithinRange && parentDocId != null) {
      ...
      await denunciasCollection.doc(parentDocId).collection('sub_denuncias').add({
        ...
        'longitude': longitude,
        'timestamp':
            FieldValue.serverTimestamp(), // Corrigido para salvar como Timestamp
        'createdAt': userEmail, // Adicionado o email do usuário
      });

      // Atualiza a denúncia principal
      ...
      // Adiciona a sub_denúncia na nova denúncia
      await denunciasCollection.doc(documentId).collection('sub_denuncias').add({
        'min': minDb.value,
        'max': maxDb.value,
        'avg': avgDb.value,
        'latitude': latitude,
