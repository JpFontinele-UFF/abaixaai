...
class _MeasurementPageState extends State<MeasurementPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: countdown == 0, // Só permite voltar quando não estiver gravando
      onPopInvoked: (didPop) {
        if (!didPop && countdown > 0) {
          // Mostra um aviso quando tenta voltar durante a gravação
          Get.snackbar(
            "Gravação em andamento",
            "Aguarde a gravação terminar para voltar",
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () {
              if (countdown > 0) {
                // Mostra aviso se estiver gravando
                Get.snackbar(
                  "Gravação em andamento",
                  "Aguarde a gravação terminar para voltar",
                  backgroundColor: Colors.orange,
